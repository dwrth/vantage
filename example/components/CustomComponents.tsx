...
export const CustomButton = ({ label, color, onClick }: { label?: string; color?: string; onClick?: () => void }) => {
  return (
    <button
      onClick={onClick}
      style={{
...
export const CustomCard = ({ title, description, imageUrl }: { title?: string; description?: string; imageUrl?: string }) => {
  return (
    <div
      style={{
        width: '100%',
...
export const Alert = ({ message, type }: { message?: string; type?: 'success' | 'error' | 'info' }) => {
  const colors = {
...
export const customComponents: ComponentRegistry<'button' | 'card' | 'counter' | 'form' | 'alert' | 'todo'> = {
  button: CustomButton,
  card: CustomCard,
  counter: Counter,
  form: CustomForm,
