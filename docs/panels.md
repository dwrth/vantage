...
| Read BP-merged layout data                   | `mergeBreakpointItemData<TData>(item, section, breakpoint)`                      |
| Read effective data (entity ∪ layout)        | `resolveEffectiveItemData` / `useItemData`                                       |
| Import / export                              | `exportLayout(layout, registry)`, `importLayout(raw, registry)`, `isValidLayout` |
| Strip / hydrate entity payloads              | `stripData`, `hydrate`                                                           |
