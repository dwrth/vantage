...
export function FormPreview({ item }: PreviewRendererProps<FormData>) {
  return <StatefulForm item={item} interactive={false} />;
}

export function FormEdit({ item, interactive }: EditRendererProps<FormData>) {
  return <StatefulForm item={item} interactive={interactive} />;
}
