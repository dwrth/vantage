...
export const defaultComponents: ComponentRegistry<'image' | 'text' | 'video'> = {
  ...
  text: ({ text }: { text: string }) => (
    <div style={{ width: '100%', height: '100%', padding: '8px', overflow: 'auto' }} contentEditable suppressContentEditableWarning>
      {text}
    </div>
  ),
  video: ({ url }: { url: string }) => (
    url ? (
      <iframe src={url} style={{ width: '100%', height: '100%', border: 0 }} allowFullScreen />
    ) : (
      <div style={{ width: '100%', height: '100%', display: 'flex', alignItems: 'center', justifyContent: 'center', background: '#f3f4f6', color: '#9ca3af' }}>
        No video URL
      </div>
